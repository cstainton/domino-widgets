...
The compatibility work therefore follows the existing port's browser/service boundaries while retaining the original widget rendering, state, event and lifecycle code. One general correctness fix was made in a separate source-fork branch: integral epoch milliseconds in `DominoId` prevent scientific notation from producing invalid CSS selectors.
